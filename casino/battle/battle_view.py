...
class BattleView(View):
    ...
    async def randomize_player_animation(self):
        ...
        while count >= 0:
            ...
            if count % 2 == 0:
                self.embed.set_field_at(index=2, name="Current Winner:", value= self.ctx.author.name, inline=False)
            else:
                self.embed.set_field_at(index=2, name="Current Winner:", value= self.challenger.name, inline=False)
            self.embed.set_field_at(index=3, name="Results", value= "The fight is happening!", inline=False)
            await self.msg.edit(embed=self.embed, view=None)
    ...
    async def pick_random_player(self):
        ...
        if number == 1:
            ...
            self.embed.set_field_at(index=2, name="State", value= winner_str, inline=False)
            self.embed.set_field_at(index=3, name="Results", value=self.ctx.author.name + " wins " + str(self.battle.credits) + " credits!", inline=False)
            ...
            self.embed.set_field_at(index=2, name="State", value= winner_str, inline=False)
            self.embed.set_field_at(index=3, name="Results", value=self.challenger.name + " wins " + str(self.battle.credits) + " credits!", inline=False)
    ...
    async def on_timeout(self):
        if not self.ended:
            self.embed.set_field_at(index=2, name="Results", value=self.challenger.name + " took too long to respond. This game has been concluded.", inline=False)
            self.embed.color = Color.yellow()
            await self.msg.edit(embed=self.embed, view=None)
